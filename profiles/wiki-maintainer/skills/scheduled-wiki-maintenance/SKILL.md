...
Read the cron prompt as the scope of this run. Load maintain-obsidian-wiki and audit-vault-links. Use /vault through the Docker tools; the host vault is /srv/obsidian. Keep LCM, offline execution and current mounts unchanged.

Use /vault/.hermes-maintenance/ for a checkpoint and run reports. Exclude this directory and .hermes-backups from content/link indexing. At the start, read the prior checkpoint if present. For the first run, inventory relevant Markdown notes and follow the prompt's scope; do not infer that a whole-vault rewrite is authorized.
...
Write a dated report under /vault/.hermes-maintenance/reports/ with examined and changed paths, validation, unresolved issues and remaining work. Save checkpoint updates atomically only for successfully processed files. On partial failure retain the previous checkpoint for unfinished items. Return a concise report with host paths and any needed follow-up; do not use Kanban completion tools without a real assigned task.
